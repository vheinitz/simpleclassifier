...
float KVStore::fget(QString name) const
{
    return get(name).toFloat();
}

QImage KVStore::imgget(QString name) const
{
    return qvariant_cast<QImage>( get(name) );
}

/*QJsonObject KVStore::jget(QString name) const
{
    QJsonDocument tmp = QJsonDocument::fromJson(get(name).toByteArray());
    return tmp.object();

}*/

QObject * KVStore::oget(QString name) const
{
    if ( get(name).canConvert<QObject*>() )
    {
        return get(name).value<QObject*>();
    }
    return 0;
}
...
void KVStore::push (QString name, QVariant val, bool persistent)
{
    _kvstore[name];
    _kvstore[name]._isPersistent = persistent;
    _kvstore[name].setValue(val);
}
...
bool KVStore::setMin (QString name, float val)
{
	TProcessData::iterator i = _kvstore.find(name);
    if ( i == _kvstore.end())
    {
    ...
    else
    {
        ...
        foreach( PDReceiverObject rec, _kvstore[name]._receivers )
        {
            rec._instance->setProperty("setMin", val);
        }
    }
    return true;
}

bool KVStore::setMax (QString name, float val)
{
	TProcessData::iterator i = _kvstore.find(name);
    if ( i == _kvstore.end())
    {
    ...
    else
    {
        ...
        foreach( PDReceiverObject rec, _kvstore[name]._receivers )
        {
            rec._instance->setProperty("setMax", val);
        }
    }
    return true;
}
