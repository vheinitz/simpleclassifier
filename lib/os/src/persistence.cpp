...
bool Persistence::declarePersistentItem( const QString &tag, double *member_var, QObject * containingObject )
{
    _doubleItems[tag] = member_var;
    QSettings s;
    if( s.contains(tag) )
    {
       *member_var = s.value(tag).toInt();
    }
    return true;
}
...
{
    QSettings s;
    foreach(QString tag, _doubleItems.keys())
    {
        s.setValue(tag, *_doubleItems[tag]);
    }
