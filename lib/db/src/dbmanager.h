...
#ifdef BUILDING_DB_DLL
#   define DB_EXPORT Q_DECL_EXPORT
#else
#   define DB_EXPORT
#endif

#define _dbm DbManager::instance()

class DB_EXPORT DBObject
{
public:
	static DBObject null;
    DBObject( QString otype=QString() ):_type(otype){/*_["id"]=-1;*/}
	int _key;
	QString _type;
	QMap<QString, QVariant> _;
	int i(QString key) {return _[key].toFloat();}
	float f(QString key) {return _[key].toFloat();}
	QString s(QString key) {return _[key].toString();}
	QList<DBObject> _children;
	QString toJSON() const;
	bool contains( const QString &name)const{return _.contains(name);}
};

class DB_EXPORT DbManager
{
...
public:
	...
	DBObject getObject( QString tname, QString key, QString value );
    ...
    QList<DBObject> getObjects( QString otype, const QString & sqlcondition = QString(), const QString & orderBy = QString() );
