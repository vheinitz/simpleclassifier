...
#ifdef BUILDING_IMGDB_DLL
#   define IMGDB_EXPORT Q_DECL_EXPORT
#else
#   define IMGDB_EXPORT
#endif
...
*/
class IMGDB_EXPORT ImageDatabase
{
	QString _rootPath;
	QStringList _added;
...
public:
	...
	QString addImage( QImage img, bool *existed=0 );
	QString addImage( QString img, bool *existed=0 );
    QStringList addImages( QStringList images );
	bool removeImage( QString hash );
	bool contains( QString hash );
};

class IMGDB_EXPORT ImageCache
{
	ImageCache(){}
